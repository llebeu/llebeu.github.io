...
import { updatePost, getPostBySlug } from "@/lib/posts";
...
export default function EditPostPage() {
  ...
  useEffect(() => {
    const loadPost = async () => {
      try {
        // 실제로는 ID로 포스트를 가져와야 하지만,
        // 현재 구조에서는 slug로 가져와야 합니다.
        // 이 부분은 나중에 개선이 필요합니다.
  ...
  if (isLoadingPost) {
    return (
      <div className="container mx-auto px-6 py-8">
        <div className="text-center">포스트를 불러오는 중...</div>
      </div>
    );
  }
  ...
  return (
    <div className="container mx-auto px-6 py-8 max-w-4xl">
      <div className="mb-8">
        <h1 className="text-3xl font-bold mb-2">포스트 편집</h1>
        <p className="text-muted-foreground">포스트를 수정하세요</p>
      </div>
